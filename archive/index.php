...
try { $selection=archive_selection($_GET); $view=archive_catalog_view($selection); }
...
<?php if (!$visible): ?><div class="empty-state"><h3>No posts here yet</h3><p><?=$counts[$selection['type']]?'There are no posts for this date. Choose another month or browse all dates.':'This feed will fill as posts are added to the archive.'?></p><a href="<?=esc(archive_link($selection,['year'=>'','month'=>'','day'=>'','page'=>1,'type'=>$counts[$selection['type']]?$selection['type']:'all']))?>">Browse <?=$counts[$selection['type']]?'all dates':'all posts'?> →</a></div><?php endif; ?>
