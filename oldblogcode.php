<?php include 'head.php'; ?>
</head>
<?php include 'header.php'; ?>
    </header>
  <main class="longreads">   
  <article class="h-entry">
  <h1 class="p-name">Old Blog Code</h1>
  <div class="pubinfo">
  <p>Published by <a class="p-author h-card" href="https://jgregorymcverry.com">J. Gregroy McVerry</a> 
     on <time class="dt-published" datetime="2020-02-22 10:54:20">22<sup>nd</sup> February 2020</time></p>
    <small>Image: Screenshot by me shared under a <a href="https://creativecommons.org/licenses/by-sa/2.0/">Creative Commons (BY-SA) license</a> </small>

    
    <span class="summary">
    <p class="p-summary">A  Remix #poetryport gift for #IndieWeb Community gathering at @IndieWebCamp Austin for all you do, and as areminder me skipping Karoke is gift to world.</p>
    </span>
  </div>
<img class="featured u-featured" src="https://i1.wp.com/clmooc.com/wp-content/uploads/2020/01/PoetRY_Port_LogoYes-lowres-e1580420665271.png?fit=800%2C800&ssl=1" alt="screenshot of my notes">
    <div class="copy">

  <div class="e-content">
    <video  controls  poster="https://indieweb.org/images/thumb/6/67/indiewebcamp-logo-lockup-color%403x.png/1200px-indiewebcamp-logo-lockup-color%403x.png" preload="metadata">
            <source src="https://jgregorymcverry.com/videos/oldblogcode.mp4" type="video/mp4">
               <track label="English" kind="subtitles" srclang="en" src="oldblogcode.sbv" default>
          </video>
     
      </div>
<p><small>Also on <a class="u-syndication" href="https://indieweb.xyz/en/poetry">IndieWeb Poetry</a></small> <small>Also on <a class="u-syndication" href="https://clmoocring.jgregorymcverry.com/clmooc-planet/">CLMOOC Planet</a></small><br /><a class="u-syndication" href="https://brid.gy/about#twitter">Published with Bridgy</a></p>
</article>
    </main>
    <footer>
<?php include 'footer.php'; ?>

  </body>
 
</html>
