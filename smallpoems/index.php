...
try { $selection=archive_selection($_GET); $entries=smallpoems_entries(); }
...
if ($selection['page']>$pages) { http_response_code(404); echo '<h1>Page not found</h1><p><a href="'.esc(smallpoems_link($selection,['page'=>1])).'">Return to this feed</a></p>'; exit; }
$visible=array_slice($matching,($selection['page']-1)*20,20);
$title='Small Poems'.($selection['type']==='all'?'':' · '.$types[$selection['type']]);
...
$feed=smallpoems_link($selection,['page'=>1],'/smallpoems/feed.php');
?><!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title><?=esc($title.($period?' · '.$period:'').' · Greg McVerry')?></title>
<meta name="description" content="Poems and posts tagged #smallpoems, collected from Greg McVerry’s archive.">
<link rel="canonical" href="<?=esc('https://jgregorymcverry.com'.smallpoems_link($selection))?>">
...
<div class="archive-heading"><div><p class="eyebrow">Greg McVerry / Writing &amp; conversations</p><h1><?=esc($title)?></h1><p class="intro">Little poems, gathered over time. Browse posts tagged #smallpoems by type and date.</p></div><a class="rss-link" href="<?=esc($feed)?>">Subscribe via RSS ↗</a></div>
<div class="archive-layout"><aside class="archive-sidebar"><nav aria-label="Post types"><h2>Post types</h2><ul class="type-list">
<?php foreach ($types as $key=>$label): ?><li><a href="<?=esc(smallpoems_link($selection,['type'=>$key,'page'=>1]))?>" <?=$selection['type']===$key?'aria-current="page"':''?>><span><?=esc($label)?></span><span class="count"><?=$counts[$key]?></span></a></li><?php endforeach; ?>
</ul></nav></aside><div class="archive-main">
<section class="date-browser" aria-labelledby="date-heading"><div class="section-top"><h2 id="date-heading">Browse by date</h2><a href="<?=esc(smallpoems_link($selection,['year'=>'','month'=>'','day'=>'','page'=>1]))?>">All dates</a></div>
<nav aria-label="Years" class="date-links"><?php foreach ($years as $year=>$count): ?><a href="<?=esc(smallpoems_link($selection,['year'=>(string)$year,'month'=>'','day'=>'','page'=>1]))?>" <?=$selection['year']===(string)$year?'aria-current="date"':''?>><?=esc($year)?> <span><?=$count?></span></a><?php endforeach; ?></nav>
<?php if (!$months): ?><p class="muted">No dated posts in this feed yet.</p><?php endif; ?>
<?php if ($selection['year']): ?><nav aria-label="Months in <?=esc($selection['year'])?>" class="month-grid"><?php for ($m=1;$m<=12;$m++): $mk=$selection['year'].'-'.sprintf('%02d',$m); $count=$months[$mk]??0; ?><div><?php if ($count): ?><a href="<?=esc(smallpoems_link($selection,['month'=>sprintf('%02d',$m),'day'=>'','page'=>1]))?>" <?=$selection['month']===sprintf('%02d',$m)?'aria-current="date"':''?>><?=date('M',strtotime($mk.'-01'))?> <span><?=$count?></span></a><?php else: ?><span class="inactive"><?=date('M',strtotime($mk.'-01'))?> <span>0</span></span><?php endif; ?></div><?php endfor; ?></nav><?php endif; ?>
...
<?php for ($cell=0;$cell<(int)ceil(($offset+$length)/7)*7;$cell++): if ($cell%7===0) echo '<tr>'; $d=$cell-$offset+1; ?><td><?php if ($d>0 && $d<=$length): $day=sprintf('%02d',$d); $count=$days[$base.'-'.$day]??0; if ($count): ?><a href="<?=esc(smallpoems_link($selection,['day'=>$day,'page'=>1]))?>" aria-label="<?=esc(archive_date_label($base.'-'.$day).', '.$count.' posts')?>" <?=$selection['day']===$day?'aria-current="date"':''?>><span><?=$d?></span><small><?=$count?></small></a><?php else: ?><span class="inactive"><?=$d?></span><?php endif; endif; ?></td><?php if ($cell%7===6) echo '</tr>'; endfor; ?></tbody></table><?php endif; ?>
...
<?php if (!$visible): ?><div class="empty-state"><h3>No posts here yet</h3><p><?=count($typed)?'There are no posts for this date. Choose another month or browse all dates.':'This feed will fill as posts are added to the archive.'?></p><a href="<?=esc(smallpoems_link($selection,['year'=>'','month'=>'','day'=>'','page'=>1,'type'=>count($typed)?$selection['type']:'all']))?>">Browse <?=count($typed)?'all dates':'all posts'?> →</a></div><?php endif; ?>
...
<?php if ($pages>1): ?><nav class="pagination" aria-label="Feed pages"><?php if ($selection['page']>1): ?><a rel="prev" href="<?=esc(smallpoems_link($selection,['page'=>$selection['page']-1]))?>">← Newer</a><?php else: ?><span></span><?php endif; ?><span>Page <?=$selection['page']?> of <?=$pages?></span><?php if ($selection['page']<$pages): ?><a rel="next" href="<?=esc(smallpoems_link($selection,['page'=>$selection['page']+1]))?>">Older →</a><?php else: ?><span></span><?php endif; ?></nav><?php endif; ?>
